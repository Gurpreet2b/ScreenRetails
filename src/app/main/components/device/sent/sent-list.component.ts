...
export class SentListComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.authService.SetRestaurantName(`Sent List`);
    this.GetSentMediaList(1);
  }
  ...
  formSentMedia = new FormGroup({
    search: new FormControl(''),
  });

  onSearchSentMedia(formValue: any) {
    this.GetSentMediaList(1);
    this.currentPage = 1;
  }
  ...
  PageJump: any = 10;
  PageTotalNumber: any = [];
  GetSentMediaList(page: number) {
    this.loading = true;
    const formData = new FormData();
    const formValue = this.formSentMedia.value;
    const search = formValue.search || '';
    formData.append('search', search);

    let params = new HttpParams();
    params = params.append('page', page.toString())
    this.http.get(`digital_signage/dashboard_list/?search=${search}`, null, { params: params }).subscribe((res: any) => {
      const responseData = res;
      if (res.status === true) {
        this.totalItems = res.count;
        this.SentMediaList = res.data;
        this.loading = false;
